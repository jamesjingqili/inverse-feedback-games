    ...
    for t in horizon(g):-1:1
        for (ii, udxᵢ) in enumerate(uindex(g))
            for (jj, udxⱼ) in enumerate(uindex(g))
                if jj == ii
                    # @infiltrate
                    λ[(t-1)*nx*N+(ii-1)*nx+1 : (t-1)*nx*N+ii*nx] += dynamics(g)[t].A'*(λ[t*nx*N+(ii-1)*nx+1 : t*nx*N+ii*nx] + (player_costs(g)[t][ii].Q*x[t+1] + player_costs(g)[t][ii].l))
                else
                    λ[(t-1)*nx*N+(ii-1)*nx+1 : (t-1)*nx*N+ii*nx] += -strategies[t].P[udxⱼ,:]'*(dynamics(g)[t].B[:,udxⱼ]'*(λ[t*nx*N+(ii-1)*nx+1 : t*nx*N+ii*nx]+ (player_costs(g)[t][ii].Q*x[t+1] + player_costs(g)[t][ii].l) )  +
                                player_costs(g)[t][ii].R[udxᵢ, udxⱼ]*(-1)*strategies[t].P[udxⱼ,:]*x[t+1] +  player_costs(g)[t][ii].r[udxⱼ]  )
                end
            end
        end
    end
...
function solve_lq_game_FBNE_with_costate_gradient!(strategies, g, x0)
    # extract control and input dimensions
    nx = n_states(g)
    nu = n_controls(g)
    T = horizon(g)
